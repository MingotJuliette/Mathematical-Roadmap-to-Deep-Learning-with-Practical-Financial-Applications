...
## Hierarchical Embedding of Financial Texts
...
Hawkishness captures the degree of contractionary or aggressive monetary policy language:
...
$$
\text{Hawkishness}*\text{chunk} = P*\text{negative} - P_\text{positive}
$$

4. The **document-level hawkishness** is obtained by averaging scores across all chunks in the document:

$$
\text{Hawkishness}*\text{doc} = \frac{1}{N*\text{chunks}} \sum_{i=1}^{N_\text{chunks}} \text{Hawkishness}_\text{chunk,i}
$$

---

## Temporal Analysis of Document Shift

To capture policy evolution over time, the **document shift** is computed as the cosine distance between consecutive document embeddings:

$$
\text{DocShift}*t = 1 - \frac{\mathbf{v}*\text{doc}^{(t-1)} \cdot \mathbf{v}*\text{doc}^{(t)}}{|\mathbf{v}*\text{doc}^{(t-1)}| , |\mathbf{v}_\text{doc}^{(t)}|}
$$
...
* **FOMC Statements** – Released after each meeting (approximately eight per year).
* **Summary of Economic Projections (SEP)** – Published quarterly with forecasts for GDP, unemployment, inflation, and federal funds rate.

For each document, metadata includes:

* Publication and meeting dates
* Document type
* File paths to raw text

Data is extracted and cleaned programmatically to ensure reproducibility and consistent formatting for downstream processing.
...
## Application of Sentiment Features to MOVE and Gold Indices

This section describes the methodology used to link FOMC sentiment features with market volatility and gold prices. The goal is to analyze how monetary policy language affects market expectations, particularly for the **MOVE Index** and gold.
...
**Monthly aggregation:**
Since sentiment variables are derived from FOMC releases (statements and projections), we compute **monthly averages** of MOVE and gold prices to align with the frequency of available textual data:
...
### 4. Forward Stepwise OLS with Train/Test Split
...
### 5. Event Study / Intramonth Analysis
